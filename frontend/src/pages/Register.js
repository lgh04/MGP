...
function Register() {
  ...
  const handleChange = (e) => {
    ...
    if (name === "nickname") {
      ...
      const timer = setTimeout(async () => {
        ...
        try {
          const res = await fetch(`http://3.107.27.34:8000/api/check-nickname?nickname=${value}`);
  ...
  const sendEmailCode = async () => {
    try {
      const res = await fetch("http://3.107.27.34:8000/api/send-email-code", {
        method: "POST",
  ...
  const verifyEmailCode = async () => {
    try {
      const res = await fetch("http://3.107.27.34:8000/api/verify-email-code", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch("http://3.107.27.34:8000/api/register", {
        method: "POST",
        headers: {
