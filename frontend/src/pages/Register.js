...
function Register() {
  ...
  const handleChange = (e) => {
    ...
    if (name === "nickname") {
      ...
      const timer = setTimeout(async () => {
        ...
        try {
          const res = await fetch(`http://localhost:8000/users/check-nickname?nickname=${value}`);
  ...
  const sendEmailCode = async () => {
    try {
      const res = await fetch("http://localhost:8000/users/send-email-code", {
        method: "POST",
  ...
  const verifyEmailCode = async () => {
    try {
      const res = await fetch("http://localhost:8000/users/verify-email-code", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
